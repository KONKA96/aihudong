...
<body>
	...
	function saveInfo(){
		$.ajax({
			url:"/aihudong-web/admin/insertAdminScreen",
			data:$("#editFormA").serialize(),
			type:"post",
			success:function(data){
				if(data=='success'){
					alert("操作成功！");
					window.location="/aihudong-web/screen/screenDistribute";
				}else if(data=='error'){
	...
	function updateInfo(){
		$.ajax({
			url:"/aihudong-web/admin/updateScreenNum",
		...
		function showAdminInfo(id){
			$("#tbody").empty();
			$.ajax({
				url:"/aihudong-web/admin/getAdminInfo",
				data:"id="+id,
				type:"post",
				dataType:"json",
				async:false,
