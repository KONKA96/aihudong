                ...
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                    	<div  class="dataTables_wrapper form-inline" role="grid">
                    		<!-- 查询条件 -->
	                    	<form method="post" id="searchForm" action="/aihudong-web/teacher/showAllTeacher">
		                    	<div class="row">
		                    		<div class="col-sm-2">
		                    			<div class="dataTables_length">
		                    				<a href="/aihudong-web/teacher/toUpdate" class="btn btn-primary ">新增用户</a>
		                    			</div>
		                    		</div>
		                    		<div class="col-sm-10">
										...
										<div class="input-group">
											<select class="form-control" id="subjectSelected"
												...
												<option value="">--请选择--</option>
												<c:forEach items="${facultyList }" var="faculty">
													<c:if test="${requestScope.teacher.subject.faculty.id==faculty.id}">
														<c:forEach items="${faculty.subjectList }" var="subject">
															<option value="${subject.id}"
																${requestScope.teacher.subjectId==subject.id ? 'selected' : '' }>${subject.subjectName }</option>
														</c:forEach>
													</c:if>
												</c:forEach>
											</select>
                    		...
                    		<form method="POST"  enctype="multipart/form-data" id="form1" action="/aihudong-web/teacher/uploadExcel">  
						        <table>  
						        <tr>  
	                            ...
	                            <tbody>
	                            	<c:forEach items="${teacherList }" var="teacher">
	                            		<tr class="gradeA">
			                                 ...
			                                 <td>
			                                 	<a href="/aihudong-web/teacher/toUpdate?id=${teacher.id }"><i style="margin-left:5px;" class="fa fa-edit"></i></a>
							...
							<jsp:include page="../common/include_page.jsp">
 								<jsp:param value="/aihudong-web/teacher/showAllTeacher" name="pageTitle"/>
							</jsp:include>
							<!-- 分页结束 -->
                    </div>
                    </div>
...
function changeSubject(object){
	...
	function deleteTeacher(id){
		...
		if(f){
			$.ajax({
				url:"/aihudong-web/teacher/deleteTeacher",
				data:"id="+id,
				type:"post",
